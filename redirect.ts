...
export function performRedirect () {
  return ({ query }: Request, res: Response, next: NextFunction) => {
    ...
    // Перевірка, чи є URL дозволеним
    if (security.isRedirectAllowed(toUrl) && isValidUrl(toUrl)) {
...
// Функція перевірки валідності URL
function isValidUrl(toUrl: string): boolean {
  try {
    const parsedUrl = new URL(toUrl);
    
    // Перевірка дозволених доменів
    const allowedDomains = ['example.com', 'anothertrusted.com']; // Це список дозволених доменів
    const domain = parsedUrl.hostname;

    return allowedDomains.includes(domain) && parsedUrl.protocol === 'https:';
  } catch (error) {
    return false;
  }
}
