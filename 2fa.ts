...
export async function status (req: Request, res: Response) {
  try {
    ...
    if (user.totpSecret === '') {
  // Генерація секрету на сервері
  const secret = otplib.authenticator.generateSecret();

  // Генерація одноразового токену для налаштування
  const setupToken = security.authorize({
    secret,
    type: 'totp_setup_secret',
    expiresIn: '10m'  // Токен діє лише 10 хвилин
  });
  ...
  // Інша логіка для налаштованої автентифікації
}
 else {
      res.json({
        setup: true
